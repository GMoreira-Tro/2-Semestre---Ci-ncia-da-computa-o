...
public class StaticQueue<E> implements MyQueue<E>{
	...
	public E dequeueV2() throws UnderflowException {
		if (isEmpty())
			throw new UnderflowException("Underflow try at queue.");
		E element = elements[first];
		elements[first] = null; // p/ coleta de lixo
		if (first == last)
			first = last = -1;
		else if (first+1 < elements.length)
			first = first + 1;
		else
			first = (first+1 - elements.length);
		return element;
	...
	public void enqueueV2(E element) throws Exception {
